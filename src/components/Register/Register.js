...
// *********** To get the name, email and password from the Regisration form ************
  onNameChange = (event) => {
...
// When the Register button is click the user info will be sent to Database
  // onSubmitSignIn = () => {
  //   fetch('http://localhost:2900/register', {
  //     method: 'post',
  ...
  // }
    ...
    // Lecture 316 - Connecting to Remote Heroku Database Server
    // My Heroku Server URL: https://nameless-temple-56098.herokuapp.com/
        //fetch('http://localhost:2900/register', {//https://nameless-temple-56098.herokuapp.com/
  onSubmitSignIn = () => {
    fetch('https://nameless-temple-56098.herokuapp.com/register', {
      method: 'post',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
        email: this.state.email,
      ...
      .then(response => response.json())
      .then(user => {
        if (user.id) { // If we receive this user info back from Database then we can go to Home page
          this.props.loadUser(user)
          this.props.onRouteChange('home');
        }
      })
        ...
        <main className="pa4 black-80">
          <div className="measure">
            <fieldset id="sign_up" className="ba b--transparent ph0 mh0">
              <legend className="f1 fw6 ph0 mh0">New Register Page</legend>
              <div className="mt3">
...
// ************************** This is the Working code Section before connecting to the Database *****************************
